...
1. **顶部控制栏**：包含返回按钮、当前页标题、保存按钮
2. **左侧导航栏**：页签式导航，切换不同的卡片类型或模块
3. **中央编辑区**：核心编辑区域，用于输入和修改各项表单数据
4. **右侧预览区**：显示卡片预览效果、绑定效果/固词描述

### 首页采用 3 栏布局设计：

1. **左侧导航**：快速访问套组管理和全局库
2. **中央内容区**：套组列表和管理操作
3. **右侧信息栏**：统计信息、快速操作和帮助提示
...
├── run.py                          # 启动脚本
├── requirements.txt                # Python 依赖清单（向后兼容）
├── pyproject.toml                  # Python 项目配置（推荐使用）
